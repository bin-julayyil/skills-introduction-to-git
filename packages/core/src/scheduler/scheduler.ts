...
export class Scheduler {
  ...
  private async _processToolCall(
    ...
    if (hookSystem) {
      ...
      const toolInput = (toolCall.invocation.params || {}) as Record<string, unknown>;
      ...
      if (beforeOutput) {
        if (beforeOutput.shouldStopExecution()) {
          this.state.updateStatus(
            ...
            createErrorResponse(
              toolCall.request,
              new Error(`Agent execution stopped by hook: ${beforeOutput.getEffectiveReason()}`),
          ...
          if (modifiedInput) {
            ...
            try {
              ...
              this.state.updateStatus(
                ...
                createErrorResponse(
                  toolCall.request,
                  new Error(`Tool parameter modification by hook failed validation: ${error instanceof Error ? error.message : String(error)}`),
                  ToolErrorType.INVALID_TOOL_PARAMS,
                ),
              );
              return;
    ...
    // Policy & Security
    const { decision: policyDecision, rule } = await checkPolicy(toolCall, this.config);
